...
import com.stv.widget.adapter.OnChildSelectedListener;
...
public class GridManager extends GridLayoutManager {
    ...
    @Override
    public boolean requestChildRectangleOnScreen(final RecyclerView parent, View child, Rect rect,
            boolean immediate) {
        mParent = parent;
        ...
        Rect childRect = new Rect(child.getLeft(), child.getTop(), child.getRight(),
                child.getBottom());
    ...
    public View onFocusSearchFailed(View focused, int focusDirection,
        ...
        if (orientation == VERTICAL) {
            ...
            if (focusParent != null && focusParent.getTag() != null) {
    ...
    private int findLastCompletelyVisiblePosition() {
        int lastVisiblePosition = RecyclerView.NO_POSITION;
        try {
            lastVisiblePosition = findLastCompletelyVisibleItemPosition();
        } catch (NullPointerException exception) {
            View view = getChildAt(getChildCount() - 1);
            lastVisiblePosition = (Integer) view.getTag();
        }
